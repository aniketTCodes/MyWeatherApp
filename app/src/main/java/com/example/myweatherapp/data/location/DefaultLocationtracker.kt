...
class DefaultLocationTracker @Inject constructor(
    ...
    override suspend fun getCurrentLocation(): Location? {
        val hasAccessFineLocationTracker=ContextCompat.checkSelfPermission(
            application,
            Manifest.permission.ACCESS_FINE_LOCATION
        )==PackageManager.PERMISSION_GRANTED

        val hasAccessCoarseLocationTracker=ContextCompat.checkSelfPermission(
            application,
        ...
        if(!hasAccessCoarseLocationTracker||!hasAccessFineLocationTracker||!isGpsEnabled){
            return null
        }
